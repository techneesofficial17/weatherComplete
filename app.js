window.addEventListener('load', () => {
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            const api = `http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=0be1e7e3cfbb30443197baae89a01de7`;
            async function weatherFetch(api) {
                fetch(api)
                    .then(Response => {
